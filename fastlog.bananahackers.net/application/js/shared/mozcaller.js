...
(function(global, nav) {
  var tel = nav.mozTelephony, sanRegex = /[^0-9+\*#,]+/g, tonePause = 1600
  ...
  function sendToneGroups(groups, conn, cb) {
    global.setTimeout(function() {
      sendDTMF(groups.shift()).then(function(){
  ...
  global.MozCaller = {
    ...
    dial: function(number, simId, connectionCb) {
      ...
      var dCB = function(c) {
        if(c instanceof MMICall) { //it was a USSD call, we can only return if it was successful or not
          c.result.then(function(r) {
            connectionCb(r.success)
          })
        } else if(c instanceof TelephonyCall) { //on normal call connection, return the actual TelephonyCall object
          c.addEventListener('connected', function() {
            if(numberParts.length) { //proceed if there is anything else to send after pauses
              sendToneGroups(numberParts, c, connectionCb)
            } else connectionCb(c)
          })
        }
      }
      tel.dial(baseNumber, tel.CALL_TYPE_VOICE, 0, simId).then(dCB).catch(function(e) {
