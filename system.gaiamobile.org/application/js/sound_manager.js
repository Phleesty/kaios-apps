(function(exports) {
    ...
    SoundManager.prototype.DEBUG = false;
    SoundManager.prototype.showUI = true;
    ...
    SoundManager.prototype.isActivated = function() {
    ...
    SoundManager.prototype.params = 0;
    ...
    SoundManager.prototype.start = function sm_start() {
        ...
        window.addEventListener('mozbrowserbeforekeydown', this.handleBeforeKeyDown);
        ...
        window.addEventListener(Service.query('isQwerty') ? 'hold4' : 'holdhash', this);
        ...
        var self = this;
        window.asyncStorage.getItem(SoundManager.CACHE_CETIMES, function getCETime(value) {
            if (!value) {
                return;
            } else {
                self.CEAccumulatorTime = 0;
            }
        });
        Service.registerState('isHeadsetConnected', this);
        Service.registerState('currentChannel', this);
        Service.registerState('isActivated', this);
    };
    SoundManager.prototype.stop = function sm_stop() {
        ...
        window.removeEventListener('mozbrowserbeforekeydown', this.handleBeforeKeyDown);
        window.removeEventListener(Service.query('isQwerty') ? 'hold4' : 'holdhash', this);
        ...
        Service.unregisterState('isActivated', this);
    };

    SoundManager.prototype.isSilentActive = function sm_isSilentActive() {
// Считаем silent, если notification=0 и вибрация выключена; опционально учитываем content
try {
var notifZero = (this.currentVolume && this.currentVolume.notification === 0);
var noVibrate = (this.vibrationEnabled === false);
// Если нужно, учитывай и контент:
// var contentZero = (this.currentVolume && this.currentVolume.content === 0);
// return (notifZero && noVibrate && contentZero);
return (notifZero && noVibrate);
} catch (e) {
return false;
}
};
...
break;
                ...
                switch (e.detail.type) {
                    case 'bluetooth-volumeset':
                        if (Bluetooth.isProfileConnected(Bluetooth.Profiles.SCO)) {
                            this.changeVolume(e.detail.value - this.currentVolume.bt_sco, 'bt_sco');
                        }
                        break;
                    case 'headphones-status-changed':
                        this.setHeadsetState(e.detail.state !== 'off');
                        this.ceAccumulator();
                        if ((!this.isHeadsetConnected) && (e.detail.state !== 'off') && (this.currentVolume['content'] >= this.CEWarningVol) && isChangeToSave) {
                            this.changeToSave();
                        }
                        break;
                    case 'default-volume-channel-changed':
                        ...
                        break;
                    case 'request-volume-up':
                        this.handleVolumeKey(1);
                        break;
                    case 'request-volume-down':
                        this.handleVolumeKey(-1);
                        break;
                    case 'request-volume-show':
                        this.handleVolumeKey(0);
                        break;
                }
                break;
            case 'appopen':
    ...
    SoundManager.prototype.handleVolumeKey = function sm_handleVolumeKey(offset) {
        if (Bluetooth.isProfileConnected(Bluetooth.Profiles.SCO) && this.isOnCall()) {
    ...
    SoundManager.prototype.handleBeforeKeyDown = function sm_handleBeforeKeyDown(event) {
        if (this.isActivated() && !Service.query('hasVolumeKey')) {
            ...
            if (event.key !== 'VolumeDown' && event.key !== 'VolumeUp') {
                if (event.key === '1' || event.key === '3') {
                    const window = Service.query('getTopMostWindow');
                    const origin = window && window.manifest && window.manifest.origin;
                    if (origin.indexOf('youtube') >= 0) {
                        return;
                    }
                }
        ...
        switch (this.currentChannel) {
            ...
            default:
                if (this.defaultVolumeControlChannel !== 'unknown') {
                    ...
                    return this.homescreenVisible || (Service.query('locked')) || FtuLauncher.isFtuRunning() ? 'notification' : 'content';
                }
        }
    };
    SoundManager.prototype.initVibrationUserPref = function sm_initVUserPref() {
        var self = this;
        window.asyncStorage.getItem(SoundManager.VIBRATION_USER_PREF_KEY, function onok(value) {
            if (value === null) {
                var r = SettingsListener.getSettingsLock().get(SoundManager.VIBRATION_SETTINGS_KEY);
                r.onsuccess = function get_onsuccess() {
                    self.writeVibrationUserPref(r.result[SoundManager.VIBRATION_SETTINGS_KEY]);
                };
        ...
        SettingsListener.observe(SoundManager.VIBRATION_SETTINGS_KEY, true, function(vibration) {
            var setBySelf = false;
    ...
    SoundManager.prototype.writeVibrationUserPref = function sm_writeUP(value) {
        ...
        var self = this;
        window.asyncStorage.setItem(SoundManager.VIBRATION_USER_PREF_KEY, value, function set_onsuccess() {
            self.vibrationUserPrefEnabled = value;
        });
    };
    SoundManager.prototype.ceAccumulator = function sm_ceAccumulator() {
        if (this.isHeadsetConnected && this.getChannel() === 'content' && this.currentVolume[this.getChannel()] > this.CEWarningVol) {
    ...
    SoundManager.prototype.headsetVolumeup = function sm_headsetVolumeup() {
        if ((this.currentVolume[this.getChannel()] + 1) > this.CEWarningVol && this.getChannel() === 'content') {
            if (this.CEAccumulatorTime === 0) {
                var self = this;
                var okfn = function() {
                    self.changeVolume(1);
    ...
    SoundManager.prototype.showCEWarningDialog = function sm_showCEDialog(okfn) {
        ...
        Service.request('DialogService:show', {
            header: _('ceWarningtitle'),
            content: _('ceWarningcontent'),
            translated: true,
            type: 'alert',
            onOk: () => {
    ...
    SoundManager.prototype.stopAccumulator = function sm_stopAccumulator() {
        if (this.CEAccumulatorID !== null) {
            ...
            window.asyncStorage.setItem(SoundManager.CACHE_CETIMES, this.CEAccumulatorTime);
        }
    };
        ...
        function observeSettingsVolumeChange(channel) {
            ...
            SettingsListener.observe(setting, 5, function onChange(volume) {
                var settingsChange = function settings_change() {
                    ...
                    self.currentVolume[channel] = parseInt(Math.max(0, Math.min(max, volume)), 10);
                    if (self.currentVolume[channel] !== volume) {
                        let settingObject = {};
                        settingObject['audio.volume.' + channel] = self.currentVolume[channel];
                        SettingsListener.getSettingsLock().set(settingObject);
                        return;
                    }
                    if (channel === 'content' && self.volumeFetched && volume > 0) {
                        self.leaveSilentMode('content', true);
                        if (isChangeToSave && self.isHeadsetConnected) {
                            if (self.currentVolume['content'] < self.CEWarningVol) {
                        ...
                        self.leaveSilentMode('notification', true);
                    } else if (channel === 'notification' && volume === 0) {
                    ...
                    if (channel === 'telephony' && this.isHeadsetConnected && this.currentChannel === 'telephony' && this.currentVolume['telephony'] >= this.TEWarningVol) {
                        ...
                        let highVolumeEvt = new CustomEvent('highvolume', {
                            detail: detail
                        });
                        window.ExternalScreenManager.send(highVolumeEvt);
                    }
                    if (channel === 'content' && this.isHeadsetConnected && this.getChannel() === 'content' && this.currentVolume['content'] >= this.CEWarningVol) {
                        this.debug(`Warning High Volume with headset media play`);
                        let detail = {
                            channel: 'content'
                        };
                        let highVolumeEvt = new CustomEvent('highvolume', {
    ...
    SoundManager.prototype.fetchCachedVolume = function sm_fetchCachedVolume() {
        ...
        var self = this;
        this.cachedChannels.forEach(function iterator(channel) {
            window.asyncStorage.getItem('content.volume', function onGettingCachedVolume(value) {
                if (!value) {
    ...
    SoundManager.prototype.calculateVolume = function sm_calVol(curVolume, delta, channel) {
    ...
    SoundManager.prototype.getVibrationAndMuteState = function sm_getState(delta, channel) {
    ...
    SoundManager.prototype.enterSilentMode = function sm_enterSlient(channel) {
        ...
        var isCachedAlready = (this.cachedVolume[channel] === this.currentVolume[channel]);
        ...
        var self = this;
        this.drawVolumeBar(channel, 0);
        req.onsuccess = function onSuccess() {
            self.pendingRequest.p();
            if (!isCachedAlready) {
                window.asyncStorage.setItem(channel + '.volume', self.cachedVolume[channel]);
            }
        };
    ...
    SoundManager.prototype.leaveSilentMode = function sm_leaveSlient(channel, skip_restore) {
        if (!channel) {
            channel = 'content';
        }
        if (!skip_restore && (this.cachedVolume[channel] > 0 || this.currentVolume[channel] === 0)) {
            var req;
            var settingObject = {};
            var self = this;
            settingObject['audio.volume.' + channel] = (this.cachedVolume[channel] > 0) ? this.cachedVolume[channel] : 1;
            this.pendingRequest.v();
            if (channel === this.getChannel()) {
                this.drawVolumeBar(channel, settingObject['audio.volume.' + channel]);
            }
    ...
    SoundManager.prototype.hideVolume = function sm_hideVolume() {
        if (!this.visibility) {
            return;
        }
        ...
        this.publish('soundManagerHide');
    };
    SoundManager.prototype.changeToSave = function sm_changeToSave() {
        isChangeToSave = false;
        this.currentVolume['content'] = this.CEWarningVol - 1;
        var settingObject = {};
        settingObject['audio.volume.' + 'content'] = this.CEWarningVol - 1;
        var req = SettingsListener.getSettingsLock().set(settingObject);
    };
    SoundManager.prototype.forceVibrate = function() {
        this.debug('forceVibrate.');
        this.cachedVolume['notification'] = this.currentVolume['notification'];
        this.currentVolume['notification'] = 1;
        this.changeVolume(-1, 'notification');
    };
    SoundManager.prototype.toggleVibrate = function() {
        var _channel = 'notification';
        var _currentNotificationVolume = this.currentVolume[_channel];
        if (_currentNotificationVolume || this.cachedVolume[_channel] < 0) {
            this.forceVibrate();
            this.enterSilentMode('content');
        } else {
            this.debug('leave vibration mode');
            if (this.cachedVolume[_channel] === 0) {
                this.cachedVolume[_channel] = -1;
            }
            this.changeVolume(this.cachedVolume[_channel], _channel);
            this.leaveSilentMode('content');
        }
    };
    SoundManager.prototype.drawVolumeBar = function sm_drawVolumeBar(channel, volume) {
        if (channel !== this.getChannel() && channel !== 'bt_sco') {
            return;
        }
        var volumeBar = document.getElementById('volume-bar');
        var volumeLeveItem = document.getElementById('volume-level');
        var volumeInfoTitle = document.getElementById('info-title');
        var infoIcon = document.getElementById('info-icon');
        var _localize = navigator.mozL10n.setAttributes;
        volumeBar.dataset.level = volume;
        var diameter = volume * 16 / (SoundManager.MAX_VOLUME[channel] + 1) + 6;
        volumeBar.style.width = diameter + 'rem';
        volumeBar.style.height = diameter + 'rem';
        infoIcon.classList.remove('silent', 'vibrate');
        if (volume === 0) {
            if (this.vibrationEnabled && channel !== 'content' && channel !== 'telephony' && channel !== 'alarm' && channel !== 'bt_sco') {
                infoIcon.classList.add('vibrate');
                _localize(volumeInfoTitle, 'vibrate-only');
            } else {
                infoIcon.classList.add('silent');
                _localize(volumeInfoTitle, 'silent');
            }
        } else {
            volumeInfoTitle.textContent = '';
        }
        volumeLeveItem.textContent = volume === 0 ? '' : volume + '/' + SoundManager.MAX_VOLUME[channel];
    }
    SoundManager.prototype.changeVolume = function sm_changeVolume(delta, channel) {
        channel = channel ? channel : this.getChannel();
        var vibrationEnabledOld = this.vibrationEnabled;
        var volume = this.calculateVolume(this.currentVolume[channel], delta, channel);
        window.ExternalScreenManager.send(new CustomEvent('volumechange', {
            detail: {
                channel: channel,
                volume: volume,
                max: SoundManager.MAX_VOLUME[channel]
            }
        }));
        this.muteState = this.getVibrationAndMuteState(delta, channel);
        this.currentVolume[channel] = volume = Math.max(0, Math.min(SoundManager.MAX_VOLUME[channel], volume));
    ...
    SoundManager.prototype.VOLUME_TITLE_L10N_MAPPING = {
        'content': 'volume-type-content',
        'telephony': 'tcl-volume-type-telephony',
        'alarm': 'volume-type-alarm',
        'notification': 'tcl-volume-type-notification'
    };
    SoundManager.prototype.updateVolumeTitle = function() {
        if (!this.volumeTitle) {
            this.volumeTitle = document.querySelector('.volume-title');
        }
        this.volumeTitle.dataset.l10nId = this.VOLUME_TITLE_L10N_MAPPING[this.getChannel()];
    };
    ...
    SoundManager.prototype.debug = function sm_debug() {
        if (this.DEBUG) {
